...
public class ChocolateHashMap<K, V> {
    ...
    public boolean containsKey(K key) {
        ...
        if (buckets[bucket].getNext().getEntry() != null) {
            BatchNode<ChocolateEntry<K, V>> node = buckets[bucket].getNext();
            while (!node.isSentinel()) {
                if (node.getEntry().getKey() == key || node.getEntry().getKey().equals(key)) {
                    return true;
                }
                node = node.getNext();
            }
    ...
    public boolean containsValue(V value) {
        for (int i = 0; i < buckets.length; i++) {
            if (buckets[i].getNext().getEntry() != null) {
                BatchNode<ChocolateEntry<K, V>> node = buckets[i].getNext();
                while (!node.isSentinel()) {
                    if (node.getEntry().getValue() == value
                            || node.getEntry().getValue().equals(value)) {
                        return true;
                    }
                    node = node.getNext();
                }
    ...
    public boolean put(K key, V value) {
        ...
        if (containsKey(key)) {
            ...
            BatchNode<ChocolateEntry<K, V>> b = new BatchNode<ChocolateEntry<K, V>>(entry,
                    buckets[index].getPrevious(), buckets[index]);
            b.getPrevious().setNext(b);
            buckets[index].setPrevious(b);
        }
        objectCount++;
    ...
    public void rehash(int newBucketCount) {
        BatchNode<ChocolateEntry<K, V>>[] original = buckets;
        buckets = (BatchNode<ChocolateEntry<K, V>>[]) new BatchNode[newBucketCount];
        fillArrayWithSentinels(buckets);
        objectCount = 0;
        for (int i = 0; i < original.length; i++) {
            if (original[i].getNext().getEntry() != null) {
                BatchNode<ChocolateEntry<K, V>> node = original[i].getNext();
                while (!node.isSentinel()) {
                    putWithoutRehashing(node.getEntry().getKey(), node.getEntry().getValue());
                    node = node.getNext();
                }
            }
        }
    ...
    private boolean putWithoutRehashing(K key, V value) {
        int index = whichBucket(key);
        BatchNode<ChocolateEntry<K, V>> node = buckets[index].getNext();
        while (!node.isSentinel()) {
        ...
        BatchNode<ChocolateEntry<K, V>> newNode =
                new BatchNode<>(new ChocolateEntry<>(key, value), node.getPrevious(), node);
